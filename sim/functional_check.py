...
from ..dataflow.os_dataflow import GPTVQTileOp
...
def run_gptvq_schedule_functional(
    ...
    Args:
        schedule: List of GPTVQTileOp from OSDataflow.generate_schedule.
        tile_config: GPTVQTileConfig from GPTVQTiler.
        codebook: [R, d] codebook centroids.
        indices: [M, K // d] integer weight indices.
        activation: [K, N] activation matrix.
        dequant_unit: DequantizationUnit for functional dequantization.
        systolic: OSSystolicArray for tile computation.
        M, N, K: MatMul dimensions.
        scales: optional [K // d] per-group scale factors.
        zero_points: optional [K // d] per-group zero points.
    ...
    for tile in schedule:
        ...
        # Extract scale/zero_point tile if applicable
        sc_tile = None
        zp_tile = None
        ...
        weight_tile = dequant_result.output  # [tile_m, tile_k]

        # Extract activation tile
